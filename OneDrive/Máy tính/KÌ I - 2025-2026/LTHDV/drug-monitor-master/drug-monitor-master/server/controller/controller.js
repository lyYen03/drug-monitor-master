...
exports.purchase = (req, res, next) => {
    try {
        ...
        if (!days || days <= 0) {
            return res.status(400).send({ message: "Days must be greater than 0" });
        }

        // Lấy tất cả thuốc từ DB
        Drugdb.find()
            .then(drugs => {
                // Tính toán nhu cầu thuốc
                const results = drugs.map(drug => {
                    const totalNeeded = drug.perDay * days;
                    const packsNeeded = Math.ceil(totalNeeded / drug.pack);
